...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
  ...
  $query = mysqli_query($conn, "SELECT count(nomor) as nomor FROM antrian1 WHERE tiket = '$tipeuser' AND DATE_FORMAT(datang,'%Y-%m-%d') = CURDATE() AND status='0'") or die('Ada kesalahan pada query tampil data : ' . mysqli_error($conn));
