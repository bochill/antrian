...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
  ...
  $query = mysqli_query($conn, "SELECT nomor FROM antrian1 WHERE DATE_FORMAT(datang,'%Y-%m-%d') = CURDATE() AND status='1' AND tiket = '$tipeuser' ORDER BY dilayani DESC LIMIT 1") or die('Ada kesalahan pada query tampil data : ' . mysqli_error($conn));
